...
public class InputPathDrawer : PropertyDrawer
{
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        if (!InputManager.Instance.IsInputAssigned(property.stringValue))
        {
    ...
    {
        if (InputManager.Instance.IsInputAssigned(_propertyValue)) return _standardPropertyHeight;
    ...
    {
        if (InputManager.Instance.MasterActionAsset == null) throw new System.Exception("Action map is not defined.");
        IEnumerable<InputActionMap> actionMaps = InputManager.Instance.MasterActionAsset.actionMaps;
